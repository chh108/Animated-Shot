...
void CParty2::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	if (m_pCamera) m_pCamera->CreateShaderVariables(pd3dDevice, pd3dCommandList);
}

void CParty2::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void CParty2::ReleaseShaderVariables()
{
	if (m_pCamera) m_pCamera->ReleaseShaderVariables();
}

void CParty2::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	if (dwDirection)
	{
		float SetSpeed = 0.3f;
...
void CParty2::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	if (bUpdateVelocity)
	{
...
void CParty2::Rotate(float x, float y, float z)
{
...
void CParty2::Update(float fTimeElapsed)
{
	...
	{
		CPartyManager::Get_Instance()->Get_Party2Type();	//HH
	}
	...
	SetPosition(CPartyManager::Get_Instance()->Get_Party2Pos());
	SetType(CPartyManager::Get_Instance()->Get_Party2Type());
}
...
void CParty2::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
...
void CSoundCallbackHandlerParty2::HandleCallback(void* pCallbackData, float fTrackPosition)
{
...
CAngrybotParty2::CAngrybotParty2(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
...
CAngrybotParty2::~CAngrybotParty2()
{
}

void CAngrybotParty2::OnPrepareRender()
{
...
void CAngrybotParty2::OnPlayerUpdateCallback(float fTimeElapsed)
{
...
#ifdef _WITH_SOUND_CALLBACK
void CAngrybotParty2::Move(ULONG dwDirection, float fDistance, bool bUpdateVelocity)
{
	m_pSkinnedAnimationController->SetTrackEnable(PS_IDLE, (dwDirection) ? true : false);

	CParty2::Move(dwDirection, fDistance, bUpdateVelocity);
}

void CAngrybotParty2::Update(float fTimeElapsed)
{
	CParty2::Update(fTimeElapsed);
