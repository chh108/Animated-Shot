...
COBB::COBB(ID3D12Device* pDevice, ID3D11DeviceContext* pContext)
...
HRESULT COBB::Initialize(void * pArg)
{
	...
	m_pOriginal_OBB = new BoundingOrientedBox(m_ColliderDesc.vCenter, XMFLOAT3(m_ColliderDesc.vSize.x * 0.5f,
		m_ColliderDesc.vSize.y * 0.5f,
		m_ColliderDesc.vSize.z * 0.5f), XMFLOAT4(0.f, 0.f, 0.f, 1.f));

	/* 회전에 대한 상태. */
	XMMATRIX		RotationMatrix = XMMatrixRotationX(m_ColliderDesc.vRotation.x) *
...
void COBB::Update(FXMMATRIX TransformMatrix)
{
	m_pOriginal_OBB->Transform(*m_pOBB, TransformMatrix);
}

bool COBB::Collision(CCollider * pTargetCollider)
{
...
bool COBB::Collision_OBB(CCollider * pTargetCollider)
{
	...
	{	
		...
		{
			XMVECTOR			vCenterDir = XMLoadFloat3(&OBBDesc[1].vCenter) - XMLoadFloat3(&OBBDesc[0].vCenter);

			float			fDistance[3] = { 0.0f };
...
COBB * COBB::Create(ID3D12Device* pDevice, ID3D11DeviceContext * pContext, CCollider::TYPE eColliderType)
{
	COBB*			pInstance = new COBB(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(eColliderType)))
	{
		MessageBox(nullptr, TEXT("Failed To Created : COBB"), L"System Message", MB_OK);
