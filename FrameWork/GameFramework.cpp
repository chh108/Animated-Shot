...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
    {
        if (wParam != SIZE_MINIMIZED)
        {
            ...
            if (m_pd3dCommandQueue && m_pdxgiSwapChain)
            {
                ...
                m_pdxgiSwapChain->ResizeBuffers(m_nSwapChainBuffers, m_nWndClientWidth, m_nWndClientHeight, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
                m_nSwapChainBufferIndex = m_pdxgiSwapChain->GetCurrentBackBufferIndex();
...
void CGameFramework::OnResize(int width, int height)
{
    if (m_pd3dDevice)
    {
        ...
        D3D12_VIEWPORT d3dViewport = { 0.0f, 0.0f, static_cast<float>(m_nWndClientWidth), static_cast<float>(m_nWndClientHeight), 0.0f, 1.0f };
        D3D12_RECT d3dScissorRect = { 0, 0, m_nWndClientWidth, m_nWndClientHeight };
...
void CGameFramework::BuildObjects()
{
    ...
    //CCactusoObject* pCactuso = new CCactusoObject(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature(), pCactusoModel, 1, 0);
    //CGameObject* GameObject = new CGameObject();

    m_pScene->m_pPlayer = m_pPlayer = pPlayer;
    m_pScene->m_pParty1 = m_pParty1 = pParty1;
    m_pScene->m_pParty2 = m_pParty2 = pParty2;
    //m_pCCactusoObject = pCactuso;
